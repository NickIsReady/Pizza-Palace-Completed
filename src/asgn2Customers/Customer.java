...
public abstract class Customer {
	...
	private String CustomerDeliveryType;
	/**
	 ...
	 * @throws CustomerException if name is empty or is more than 20 characters long, mobileNumber is empty, does not start with a 0 or contains letters, locationX or locationY is less than -10 or greater than 10, or type is empty
	 * 
	 */
	public Customer(String name, String mobileNumber, int locationX, int locationY, String type) throws CustomerException{
		if (name.isEmpty() || name.trim().length() == 0 || name.trim().length() > 20){
			throw new CustomerException("Name is invalid!");
		} else if (mobileNumber.isEmpty() || mobileNumber.charAt(0) != '0' || mobileNumber.matches("[0-9]+") == false){
			...
			throw new CustomerException("No type was given!");
		}
		CustomerName = name;
		CustomerMobile = mobileNumber;
		CustomerXLocation = locationX;
		CustomerYLocation = locationY;
		CustomerDeliveryType = type;
	}
